...
import { getAllStock, getStockById, createStock, updateStock, deleteStock, applyStockMovements } from "../controllers/stockController.js";
import { validateRequest, validateParams } from "../middlewares/validateMiddleware.js";
import { createStockSchema, updateStockSchema, stockMovementBatchSchema, idParamSchema } from "../validators/stockValidators.js";

const router = express.Router();
const lockAdmin = [verifyToken, verifyAdmin];

router.get("/", verifyToken, getAllStock);
router.post("/movements", verifyToken, validateRequest(stockMovementBatchSchema), applyStockMovements);
